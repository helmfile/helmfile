...
func setGlobalOptionsForRootCmd(fs *pflag.FlagSet, globalOptions *config.GlobalOptions) {
	...
	fs.BoolVar(&globalOptions.AllowFailedReleases, "allow-failed-releases", false, `continue processing other releases when one release fails; report the failed releases at the end`)
