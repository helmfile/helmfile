...
func setGlobalOptionsForRootCmd(fs *pflag.FlagSet, globalOptions *config.GlobalOptions) {
	...
	fs.BoolVar(&globalOptions.AllowFailedReleases, "allow-failed-releases", false, `allow partial errors during release processing, will continue and report individual releases that failed and continue to install the others`)
