...
func NewApplyCmd(globalCfg *config.GlobalImpl) *cobra.Command {
	...
	f.StringVar(&applyOptions.TemplateArgs, "template-args", "", `pass extra args to the helm template run by chartify during chart preparation (e.g. --template-args="--dry-run=server" to enable the helm lookup function)`)
