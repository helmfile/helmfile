if [[ ${HELMFILE_V1MODE} = true ]]; then
  ...
  yaml_overwrite_reverse=${yaml_overwrite_tmp}/yaml.template.build.yaml
  ...
  for i in $(seq 10); do
      ...
      diff -u ${v1_subhelmfile_multi_bases_with_array_values_output_dir}/result.yaml ${yaml_overwrite_reverse} || fail "\"helmfile template\" should be consistent"
      echo code=$?
  done
