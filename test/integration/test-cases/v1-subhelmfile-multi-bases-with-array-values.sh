if [[ ${HELMFILE_V1MODE} = true ]]; then
  ...
  for i in $(seq 10); do
      ...
      ${helmfile} -f ${v1_subhelmfile_multi_bases_with_array_values_input_dir}/helmfile.yaml.gotmpl template --debug -e dev &> ${yaml_overwrite_reverse} || (cat ${yaml_overwrite_reverse} && fail "\"helmfile template\" shouldn't fail")
